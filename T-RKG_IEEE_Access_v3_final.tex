...
This paper presents T-RKG (Temporal Records Knowledge Graph), a knowledge-based system with three contributions: a formal governance ontology of 47 classes and 23 object properties (delivered in OWL/Turtle as supplementary material) that encodes records as composite governance entities; a typed temporal relationship model in which hold propagation is governed by the declared semantic properties of inter-record connections; and a conflict-detection mechanism that applies ontological applicability reasoning, including a defeasibility axiom for GDPR Article~17(3) exemptions.

The substantive empirical content is two-fold. First, on per-record applicability against an independently-constructed ground truth measured across ten seeds and six scales (1K to 100K records), T-RKG attains F1 of $0.820 \pm 0.014$ under 10\% per-attribute label noise, against $0.537 \pm 0.038$ for a Siloed baseline and $0.109 \pm 0.010$ for a No-Ontology baseline; perturbing labels moves the score in the predicted direction, which is the structural rather than tautological reading of the clean-regime F1 of 1.000. Second, a SHACL Core baseline --- the strongest expressive baseline within standard RDF validation --- detects roughly 6\% of T-RKG's conflicts at 100K records and takes about $62\times$ longer to do so; the rule families it cannot express (temporal-interval intersection, defeasibility, cross-record propagation, Art.~17(3) exemption) are catalogued with their W3C-specification gaps in the supplementary material. The total-conflict comparison against attribute-blind baselines (T-RKG 52 to 4{,}653 conflicts per dataset; both baselines zero cross-domain) is reported in Table~\ref{tab:conflicts} as a structural baseline check rather than as the central evaluation result --- by Corollary~1 the outcome is forced by the proof, not measured. Hold propagation under a Conservative policy expands the hold set $3.34\times$ over direct seed identification and runs roughly $40\times$ faster than a flat-list and $177\times$ faster than a SQLite recursive-CTE baseline on identical workloads on this hardware; absolute multipliers will reshape on different hardware, but the qualitative ranking is structural.
\end{abstract}
...
\begin{enumerate}
    ...
    \item An empirical evaluation across ten random seeds and six dataset sizes (1K to 100K records). The substantive accuracy claim is the per-record applicability F1 measured against an independently-constructed ground truth: in a clean regime the No-Ontology baseline reaches F1 $0.109 \pm 0.010$ and the Siloed baseline F1 $0.577 \pm 0.040$; under 10\% per-attribute label noise T-RKG attains F1 $0.820 \pm 0.014$ against Siloed $0.537 \pm 0.038$ and No-Ontology $0.109 \pm 0.010$. A SHACL Core baseline --- the strongest expressive baseline within standard RDF validation --- detects roughly 6\% of T-RKG's conflicts at 100K records, with the rule families it cannot express catalogued against their W3C-specification gaps. Total-conflict counts (52 to 4{,}653 per dataset, of which 6 to 813 are cross-domain) are reported in Table~\ref{tab:conflicts} as a capability ablation rather than as the central evaluation result: under attribute-blind tagging the cross-domain conflicts are not detectable in principle (Corollary~1).
    \item Reproducibility: single-command reproduction of all reported numbers via \texttt{python -m experiments.run\_all}; 63 unit tests covering the ontology, the conflict detector, all baselines, the Art.~17(3) exemption logic, and ten competency questions; the OWL/Turtle ontology released as supplementary material.
\end{enumerate}
...
Industrial KG deployments are well documented (Galkin et al.~\cite{galkin2017}; Noy et al.~\cite{noy2019}; Pan et al.~\cite{pan2024}). Hogan et al.~\cite{hogan2021} is the canonical KG survey of the post-2018 period, and Bizer et al.~\cite{bizer2009,linkeddata} frame the broader Linked Data context. The framing of knowledge graphs as governance infrastructure for AI workflows has been discussed in the data-governance community, though no published system in the cross-regulation conflict-detection scope T-RKG addresses appears to exist; the work here is correspondingly narrower.
...
The substantive accuracy claim is per-record regulatory applicability under realistic input corruption. Table~\ref{tab:f1noise} reports F1 across three systems on a 10K-record corpus with 10\% i.i.d.\ label noise injected independently across the PII, jurisdiction, and public-company attributes after ground-truth snapshot. T-RKG achieves F1 of $0.820 \pm 0.014$, a 28-point gap over the Siloed baseline ($0.537 \pm 0.038$) and a 71-point gap over the No-Ontology baseline ($0.109 \pm 0.010$). The No-Ontology recall ceiling of 0.406 is the empirical realization of Proposition~4 (stated in §VII-D, where it is positioned next to the capability-ablation numbers it explains).
...
\hline
No-Ontology       & $0.063 \pm 0.006$ & $0.406 \pm 0.039$ & $0.109 \pm 0.010$ \\
Siloed            & $1.000 \pm 0.000$ & $0.406 \pm 0.039$ & $0.577 \pm 0.040$ \\
\hline\hline
\end{tabular}
\end{table}
...
System & Precision & Recall & F1 \\
\hline
T-RKG       & $0.777 \pm 0.019$ & $0.868 \pm 0.009$ & $0.820 \pm 0.014$ \\
No-Ontology & $0.063 \pm 0.006$ & $0.406 \pm 0.039$ & $0.109 \pm 0.010$ \\
Siloed      & $0.991 \pm 0.006$ & $0.369 \pm 0.035$ & $0.537 \pm 0.038$ \\
\hline\hline
\end{tabular}
\end{table}
...
Under the noised regime T-RKG loses approximately 18 F1 points. That loss is structural rather than circular: if the clean-regime F1 of 1.000 were merely a tautology produced by the generator's labeling scheme, perturbing the labels would not move the score.

\textbf{Noise-rate sensitivity.} Fig.~\ref{fig:noise_sweep} reports T-RKG's per-record F1 across noise rates from 2\% to 20\% (applied simultaneously and independently to the PII, jurisdiction, and public-company attributes). T-RKG degrades smoothly from $F1 = 0.964 \pm 0.005$ at 2\% noise to $F1 = 0.665 \pm 0.019$ at 20\% noise, a gradient consistent with attribute-level redundancy in the applicability predicate (each regulation's $\phi_\gamma$ is a conjunction over multiple attributes, so the probability that any single regulation is fully corrupted scales worse than linearly in any single attribute's flip rate). The Siloed baseline degrades from $0.569 \pm 0.039$ to $0.501 \pm 0.039$ over the same range. The No-Ontology baseline is approximately flat at $0.109$: its recall is capped below the noise floor by Proposition~4 because its applicability function does not consult the corrupted attributes at all.
...
\hline
1{,}000   & $2 \pm 3$    & $196 \pm 4$       & $52 \pm 10$       & $3.1 \pm 0.1$ \\
5{,}000   & $17 \pm 19$  & $1{,}018 \pm 66$  & $231 \pm 38$      & $15.6 \pm 0.2$ \\
10{,}000  & $36 \pm 27$  & $2{,}020 \pm 19$  & $445 \pm 57$      & $31.5 \pm 0.4$ \\
25{,}000  & $80 \pm 40$  & $5{,}083 \pm 46$  & $1{,}117 \pm 127$ & $79.8 \pm 2.7$ \\
50{,}000  & $165 \pm 53$ & $10{,}164 \pm 100$ & $2{,}302 \pm 241$ & $159 \pm 2$ \\
100{,}000 & $280 \pm 38$ & $20{,}347 \pm 261$ & $4{,}653 \pm 228$ & $326 \pm 14$ \\
\hline\hline
\end{tabular}
\end{table}

At 100K records, SHACL Core detects roughly 6\% of the conflicts T-RKG detects, and takes about $62\times$ longer to do so. The 6\% recall is not a comparison artifact; it is what SHACL Core can do by construction. To detect temporal-interval Hold-Deletion conflicts, defeasible-priority resolutions, or hold-propagation-induced conflicts, the baseline would need SHACL-SPARQL --- at which point it is no longer a SHACL baseline but a SPARQL-rule baseline, which is a different system class.
...
Table~\ref{tab:conflicts} reports conflict counts across six scales. The Siloed and No-Ontology columns are capability ablations, not competing implementations; their zero cross-domain count is a forced consequence of the corollary above, not an empirical finding. T-RKG's own cross-domain counts grow from $6 \pm 9$ at 1K to $813 \pm 44$ at 100K, scaling roughly linearly in the population of records that fall under two or more regulations.
...
\hline
1{,}000   & $52 \pm 10$       & $6 \pm 9$    & $45 \pm 14$       & 0 (det.) & 150 (det.) & 0 (det.) & $3.1 \pm 0.1$ \\
5{,}000   & $231 \pm 38$      & $56 \pm 24$  & $164 \pm 47$      & 0 (det.) & 750 (det.) & 0 (det.) & $15.6 \pm 0.2$ \\
10{,}000  & $445 \pm 57$      & $98 \pm 27$  & $313 \pm 66$      & 0 (det.) & 1{,}500 (det.) & 0 (det.) & $31.5 \pm 0.4$ \\
25{,}000  & $1{,}117 \pm 127$ & $209 \pm 46$ & $858 \pm 137$     & 0 (det.) & 3{,}750 (det.) & 0 (det.) & $79.8 \pm 2.7$ \\
50{,}000  & $2{,}302 \pm 241$ & $420 \pm 47$ & $1{,}766 \pm 225$ & 0 (det.) & 7{,}500 (det.) & 0 (det.) & $159 \pm 2$ \\
100{,}000 & $4{,}653 \pm 228$ & $813 \pm 44$ & $3{,}646 \pm 224$ & 0 (det.) & 15{,}000 (det.) & 0 (det.) & $326 \pm 14$ \\
\hline\hline
\end{tabular}
\end{table*}
...
\hline
Retention--Deletion & $98 \pm 27$  & 22.1\% \\
Priority            & $347 \pm 55$ & 77.9\% \\
Jurisdiction        & 0            & 0\% \\
Hold--Deletion      & 0            & 0\% \\
\hline
Critical severity   & $35 \pm 15$  & 7.9\% \\
High severity       & $63 \pm 20$  & 14.2\% \\
Medium severity     & $23 \pm 19$  & 5.2\% \\
Low severity        & $324 \pm 50$ & 72.7\% \\
\hline\hline
\end{tabular}
\end{table}
...
Table~\ref{tab:propagation} reports propagation results for the 10K dataset, using 50 matter-scoped seed records at depth limit~10. Starting from 50 seeds, the Conservative policy (attachments and threads) produces a mean hold set of $167 \pm 39$ records, a 3.34$\times$ expansion. Adding derivation relationships increases the expansion from 3.34$\times$ to 3.68$\times$ --- a contribution that does not change the propagation-policy ordering. Expanding to all relationship types reaches 4.55$\times$, numerically close to what an untyped graph traversal would produce, with the key difference that each record in the T-RKG hold set can be traced to the specific relationship type that caused its inclusion.
...
Attachment      & $96 \pm 20$   & 1.91$\times$ & $0.30 \pm 0.06$ \\
Thread          & $72 \pm 7$    & 1.44$\times$ & $0.21 \pm 0.02$ \\
Att + Thread    & $167 \pm 39$  & 3.34$\times$ & $0.51 \pm 0.12$ \\
+ Derivation    & $184 \pm 39$  & 3.68$\times$ & $0.54 \pm 0.11$ \\
All types       & $227 \pm 59$  & 4.55$\times$ & $0.68 \pm 0.18$ \\
\hline\hline
\end{tabular}
\end{table}
...
\caption{Propagation expansion by relationship policy. Conservative (Att + Thread) yields a 3.34$\times$ expansion; the Comprehensive policy (All types) reaches 4.55$\times$ but loses the per-record relationship-type provenance.}
...
Table~\ref{tab:ablation} reports the ablation. The experiment uses 50 matter-scoped seeds at depth 10, so the Full T-RKG hold set of 167 (3.34$\times$) here corresponds directly to the Att + Thread row in Table~\ref{tab:propagation}.
...
\hline
Full T-RKG       & \checkmark & \checkmark & \checkmark & $445 \pm 57$    & 167 (3.34$\times$) \\
No Ontology      & ---        & \checkmark & \checkmark & $1{,}500 \pm 0$ (det.) & 167 (3.34$\times$) \\
...
\begin{enumerate}
    ...
    \item run the detector twice: once with the matter dictionary plumbed in (Axiom~4 active), once without (no suppression).
\end{enumerate}
...
The composite-record formulation, expressed as ontology-driven applicability inference over typed temporal relationships, makes cross-system regulatory conflicts visible at a cost of roughly 320~ms per 100K records on a single host. Corollary~1 explains why the two ablation baselines return zero cross-domain conflicts: under attribute-blind tagging the conflicts are not detectable in principle, so the baseline numbers do not themselves quantify T-RKG's advantage. The contribution rests on what survives that disclaimer. The noised-regime F1 of $0.820 \pm 0.014$ separates T-RKG from the Siloed ($0.537 \pm 0.038$) and No-Ontology ($0.109 \pm 0.010$) baselines under 10\% per-attribute label noise. The SHACL Core baseline detects roughly 6\% of T-RKG's conflicts at 100K records and takes about $62\times$ longer to do so, with the rule families it cannot express catalogued in the supplementary material against their specification gaps. The contribution would be falsified by a production-graph-DB implementation of the same ontology that reverses the latency ranking, or by a real-corpus evaluation in which the noised-regime F1 falls below the Siloed baseline. Open items: encoding the Art.~17(3) clauses (a), (c), and (d) once purpose-of-processing predicates are materialized; a structured-noise evaluation regime; and partner-tenant integration against a real enterprise corpus.

\section*{Acknowledgment}

The author used an AI assistant for sentence-level editing on portions of the manuscript and for implementation assistance on parts of the rule-evaluation logic in \texttt{trkg/conflict.py} and the plotting code in \texttt{figures/make\_figures.py}. All experimental design, ontology authoring, axiom derivation, statistical methodology, interpretation of results, and contribution claims are the author's. AI-generated code was reviewed and integrated only after passing the unit tests in \texttt{tests/}. AI-edited prose was revised by the author. No external funding supported this work.
